...
BOT_TOKEN = os.getenv("2116211080:AAG4YeeNhLF3_fnIzJyfoZQrgl4XAI-PunU")
...
# sudo_users can restart bot and view space left on vps
...
async def download(client, message):
    try:
        ...
        await message.reply_text(text=f"**Downloaded Successfully! 🥺")
    except:
...
async def play(ctx, url: str):
    ...
    try:
        if not song_there:
            ctx.send("File not there")
    except PermissionError:
        await ctx.send("Wait for the current playing music to end or use the 'stop' command")
        return

    voiceChannel = discord.utils.get(ctx.guild.voice_channels, name='idk')
    await voiceChannel.connect()
    voice = discord.utils.get(client.voice_clients, guild=ctx.guild)
    voice.play(discord.FFmpegPCMAudio(("./cache/" + url)))
    ...
    keyboard = [
        [
            ...
            InlineKeyboardButton(
                "SHARE", url="https://t.me/share/url?url=https://t.me/AquaDlBot")
        ],
    ]
    await message.reply_text(text="<b>Hey! My name is Aqua.</b>"
